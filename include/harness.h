...
public:
  GemmHarness(std::uint32_t M, std::uint32_t N, std::uint32_t K)
      : M_(M), N_(N), K_(K),
        A_(numerics::random_matrix(M, K, 0)),
        B_(numerics::random_matrix(K, N, 1)),
        C_init_(numerics::random_matrix(M, N, 2)),
        C_ref_(C_init_),
        C_scratch_(std::size_t(M) * N),
        Ad_(A_.begin(), A_.end()),
        Bd_(B_.begin(), B_.end()),
        Cd_(C_init_.begin(), C_init_.end()) {

    const auto t0 = std::chrono::steady_clock::now();
    gemm_cpu(M_, N_, K_, kAlpha, A_.data(), B_.data(), kBeta, C_ref_.data());
    const auto t1 = std::chrono::steady_clock::now();
    const double ms =
        std::chrono::duration<double, std::milli>(t1 - t0).count();
    std::printf("[cpu reference: %.1f ms, %u x %u x %u]\n\n", ms, M_, N_, K_);
  }
  ...
  void run(const GemmKernel &kernel) {
    ...
    if (rmse > kTolerance) {
      std::printf("%-20.*s  rel RMSE %.2e  FAILED\n", int(kernel.name.size()),
    ...
    std::printf("%-20.*s  rel RMSE %.2e  %8.3f ms  %7.2f TFLOPS\n",
