...
__global__ void
double_buffer_prefetch_kernel(int M, int N, int K, float alpha, const float *__restrict__ A,
  ...
  float4 stag_A[BM / row_stride_A];
  float4 stag_B[BK / row_stride_B];

  // Load As and Bs into As[0] and Bs[0].
  const int A_col_global_0 = load_As_col * 4;
  ...
  for (int offset = 0; offset + row_stride_A <= BM; offset += row_stride_A) {
    ...
    const bool A_in = A_row_global < M && A_col_global_0 < K;

    const float4 tmp =
        loads::masked_load_f4<BoundsCheck>(&A[(load_As_row + offset) * K + load_As_col * 4], A_in);
    As[0][(load_As_col * 4 + 0) * BM + load_As_row + offset] = tmp.x;
    As[0][(load_As_col * 4 + 1) * BM + load_As_row + offset] = tmp.y;
    As[0][(load_As_col * 4 + 2) * BM + load_As_row + offset] = tmp.z;
    As[0][(load_As_col * 4 + 3) * BM + load_As_row + offset] = tmp.w;
  }

  for (int offset = 0; offset + row_stride_B <= BK; offset += row_stride_B) {
    const int B_row_global = load_Bs_row + offset;
    const bool B_in = B_row_global < K && B_col_global < N;

    reinterpret_cast<float4 *>(&Bs[0][(load_Bs_row + offset) * BN + load_Bs_col * 4])[0] =
        loads::masked_load_f4<BoundsCheck>(&B[(load_Bs_row + offset) * N + load_Bs_col * 4], B_in);
  }
  ...
  for (int k_tile = 0; k_tile < K; k_tile += BK) {
    ...
    if (has_next) {
      ...
      const int A_col_global = k_tile + BK + load_As_col * 4;
      ...
      for (int offset = 0; offset + row_stride_A <= BM; offset += row_stride_A) {
        ...
        stag_A[i] = loads::masked_load_f4<BoundsCheck>(
      ...
      int j = 0;
      for (int offset = 0; offset + row_stride_B <= BK; offset += row_stride_B) {
        const int B_row_global = k_tile + BK + load_Bs_row + offset;
        const bool B_in = B_row_global < K && B_col_global < N;
        stag_B[j] = loads::masked_load_f4<BoundsCheck>(
            &B[(load_Bs_row + offset) * N + load_Bs_col * 4], B_in);
        ++j;
      }
      A += BK;
      B += BK * N;
    ...
    for (int w_sub_row_idx = 0; w_sub_row_idx < WMITER; ++w_sub_row_idx) {
      for (int i = 0; i < TM; ++i) {
        reg_M_a[w_sub_row_idx * TM + i] =
            As[cur][(warp_row * WM + w_sub_row_idx * WSUBM + thread_row_in_warp * TM + i)];
      }
    }
    for (int w_sub_col_idx = 0; w_sub_col_idx < WNITER; ++w_sub_col_idx) {
      for (int i = 0; i < TN; ++i) {
        reg_N_a[w_sub_col_idx * TN + i] =
            Bs[cur][(warp_col * WN + w_sub_col_idx * WSUBN + thread_col_in_warp * TN + i)];
      }
    }
...
#pragma unroll
    ...
    if (has_next) {
      int i = 0;
      for (int offset = 0; offset + row_stride_A <= BM; offset += row_stride_A) {
        As[next][(load_As_col * 4 + 0) * BM + load_As_row + offset] = stag_A[i].x;
        As[next][(load_As_col * 4 + 1) * BM + load_As_row + offset] = stag_A[i].y;
        As[next][(load_As_col * 4 + 2) * BM + load_As_row + offset] = stag_A[i].z;
        As[next][(load_As_col * 4 + 3) * BM + load_As_row + offset] = stag_A[i].w;
        ++i;
      }

      int j = 0;
      for (int offset = 0; offset + row_stride_B <= BK; offset += row_stride_B) {
        reinterpret_cast<float4 *>(&Bs[next][(load_Bs_row + offset) * BN + load_Bs_col * 4])[0] =
            stag_B[j];
        ++j;
      }
    }
...
void kernels::sgemm::double_buffer_prefetch(const SgemmArgs &a) {
  ...
  cuda_utils::dispatch_bool(beta_is_zero, [&](auto beta_zero) {
    cuda_utils::dispatch_bool(!tile_aligned, [&](auto bounds_check) {
      ...
      double_buffer_prefetch_kernel<BM, BK, BN, WM, WN, WMITER, WNITER, TM, TN, kBetaZero,
                                    kBoundsCheck>
