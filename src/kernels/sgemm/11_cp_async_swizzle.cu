...
__device__ __forceinline__ int swizzle_As(int m, int k) {
  const int c = k >> 2;
  const int slot = (m * 4 + c) ^ ((m >> 3) & 7);
  return slot * 4 + (k & 3);
}
...
__global__ void cp_async_swizzle_kernel(int M, int N, int K, float alpha,
  ...
  static_assert(BK == 16, "swizzle_As is only defined for BK == 16");
  ...
  for (int offset = 0; offset + row_stride_A <= BM; offset += row_stride_A) {
    const int m = load_As_row + offset;
    __pipeline_memcpy_async(&As[0][swizzle_As(m, load_As_col * 4)], &A[m * K + load_As_col * 4],
                            sizeof(float4));
  }
  ...
  for (int k_tile = 0; k_tile < K; k_tile += BK) {
    ...
    if (has_next) {
      ...
      for (int offset = 0; offset + row_stride_A <= BM; offset += row_stride_A) {
        const int m = load_As_row + offset;
        __pipeline_memcpy_async(&As[next][swizzle_As(m, load_As_col * 4)],
                                &A[m * K + load_As_col * 4], sizeof(float4));
      }
    ...
    for (int k = 0; k < BK; ++k) {
      ...
      for (int w_sub_row_idx = 0; w_sub_row_idx < WMITER; ++w_sub_row_idx) {
        for (int i = 0; i < TM; ++i) {
          reg_M[w_sub_row_idx * TM + i] = As[cur][swizzle_As(
              warp_row * WM + w_sub_row_idx * WSUBM + thread_row_in_warp * TM + i, k)];
        }
      }
...
void kernels::sgemm::cp_async_swizzle(const SgemmArgs &a) {
  ...
  cuda_utils::dispatch_bool(beta_is_zero, [&](auto beta_zero) {
    cuda_utils::dispatch_bool(!tile_aligned, [&](auto bounds_check) {
      ...
      cp_async_swizzle_kernel<BM, BK, BN, WM, WN, WMITER, WNITER, TM, TN, kBetaZero, kBoundsCheck>
