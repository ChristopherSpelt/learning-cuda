...
__global__ void
double_buffer_prefetch_bank_kernel(int M, int N, int K, float alpha, const float *__restrict__ A,
...
#pragma unroll
    for (int i = 0; i < 4; ++i) {
      ...
      const bool A_in = A_row_global < M && load_As_k < K;

      r[i] = loads::masked_load<BoundsCheck>(&A[(m + i) * K + load_As_k], A_in);
    }
    reinterpret_cast<float4 *>(&As[0][load_As_k * (BM + 4) + m])[0] =
  ...
  for (int k_tile = 0; k_tile < K; k_tile += BK) {
    ...
    if (has_next) {
      ...
      const int A_col_global = k_tile + BK + load_As_k;
...
#pragma unroll
        ...
        stag_A[idx] = make_float4(r[0], r[1], r[2], r[3]);
        ++idx;
      }
    ...
    for (int w_sub_row_idx = 0; w_sub_row_idx < WMITER; ++w_sub_row_idx) {
      const int base = warp_row * WM + w_sub_row_idx * WSUBM + thread_row_in_warp * TM;
#pragma unroll
      for (int i = 0; i < TM; i += 4)
        reinterpret_cast<float4 *>(&reg_M_a[w_sub_row_idx * TM + i])[0] =
            reinterpret_cast<const float4 *>(&As[cur][0 * (BM + 4) + base + i])[0];
    }
...
#pragma unroll
          for (int i = 0; i < TM; i += 4)
            reinterpret_cast<float4 *>(&reg_M_b[w_sub_row_idx * TM + i])[0] =
                reinterpret_cast<const float4 *>(&As[cur][(k + 1) * (BM + 4) + base + i])[0];
        }
        for (int w_sub_col_idx = 0; w_sub_col_idx < WNITER; ++w_sub_col_idx)
          for (int i = 0; i < TN; ++i)
...
#pragma unroll
          for (int i = 0; i < TM; i += 4)
            reinterpret_cast<float4 *>(&reg_M_a[w_sub_row_idx * TM + i])[0] =
                reinterpret_cast<const float4 *>(&As[cur][(k + 2) * (BM + 4) + base + i])[0];
        }
        for (int w_sub_col_idx = 0; w_sub_col_idx < WNITER; ++w_sub_col_idx)
          for (int i = 0; i < TN; ++i)
            reg_N_a[w_sub_col_idx * TN + i] =
    ...
    if (has_next) {
      ...
      for (int offset = 0; offset + row_stride_A <= BM; offset += row_stride_A) {
        const int m = load_As_m + offset;
        reinterpret_cast<float4 *>(&As[next][load_As_k * (BM + 4) + m])[0] = stag_A[i];
        ++i;
      }
...
void kernels::sgemm::double_buffer_prefetch_bank(const SgemmArgs &a) {
  ...
  cuda_utils::dispatch_bool(beta_is_zero, [&](auto beta_zero) {
    cuda_utils::dispatch_bool(!tile_aligned, [&](auto bounds_check) {
      ...
      double_buffer_prefetch_bank_kernel<BM, BK, BN, WM, WN, WMITER, WNITER, TM, TN, kBetaZero,
                                         kBoundsCheck>
