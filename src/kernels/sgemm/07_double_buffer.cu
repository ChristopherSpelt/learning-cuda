...
__global__ void double_buffer_kernel(int M, int N, int K, float alpha, const float *__restrict__ A,
...
void kernels::sgemm::double_buffer(const SgemmArgs &a) {
  ...
  cuda_utils::dispatch_bool(beta_is_zero, [&](auto beta_zero) {
    cuda_utils::dispatch_bool(!tile_aligned, [&](auto bounds_check) {
      ...
      double_buffer_kernel<BM, BK, BN, WM, WN, WMITER, WNITER, TM, TN, kBetaZero, kBoundsCheck>
